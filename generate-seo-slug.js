...
const PATTERN_LABELS = { buy: 'شراء', price: 'سعر', kuwait: 'الكويت', hawalli: 'حولي', offer: 'عرض' };
...
for (const product of products) {
  ...
  for (const pattern of PATTERNS) {
    ...
    const slug = `${pattern}-${product.slug}`;
    pages.push({
      slug,
      productId: product.id,
      keyword: pattern,
      title: `${label} ${product.title} في الكويت | إعلانات العرب الكويت`,
      description: `${label} ${product.title} بأفضل سعر في الكويت. ${(product.description || '').substring(0, 120)}... شحن مجاني لجميع محافظات الكويت.`,
      h1: `${label} ${product.title}`,
      content: `اكتشف ${product.title} في إعلانات العرب الكويت - متجرك الموثوق. ${(product.description || '').substring(0, 500)} نوفر شحن مجاني لجميع محافظات الكويت مع ضمان الجودة.`
    });
  }
}
...
console.log(`   Sample slug: ${pages[0].slug}`);
...
for (const product of products) {
  ...
  for (const p of PATTERNS) {
    pathsArr += `    { params: { slug: '${p}-${product.slug}' } },\n`;
  }
}

const slugJsx = `import { useDispatch } from 'react-redux';
...
fs.writeFileSync(path.join(__dirname, 'src/pages/seo/[slug].jsx'), slugJsx, 'utf8');
const size = fs.statSync(path.join(__dirname, 'src/pages/seo/[slug].jsx')).size;
console.log(`✅ Generated [slug].jsx - ${size} bytes`);
console.log(`   Sample path: buy-${products[0].slug}`);
